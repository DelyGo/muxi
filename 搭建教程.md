...
2. 在 `_config.yml` 中修改：
   ...
   - `deploy.branch: gh-pages`（推荐，源码保留在 `master`，Pages 从 `gh-pages` 发布）。
...
## 六、源码备份到 GitHub（建议做一次）

博客源码只在本地的话，换电脑或误删就没了。建议把**源码**推到同一仓库的 **master** 分支（和发布用的 **gh-pages** 分开）：

```bash
cd ~/Desktop/code/muxi
git remote add origin git@github.com:DelyGo/muxi.git   # 若已添加过可跳过
git add .
git commit -m "备份博客源码"
...
```bash
git add .
git commit -m "更新文章/配置"
git push
npm run deploy
```

`.gitignore` 已忽略 `node_modules/`、`public/`、`.deploy_git/`、`db.json` 等，不会把这些推上去。
...
4. 部署：`npm run deploy`；若已备份过源码，可先 `git add . && git commit -m "..." && git push` 再 `npm run deploy`
